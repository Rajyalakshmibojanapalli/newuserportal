        ...
        <div className="relative mb-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              autoComplete="off"
              className={`w-full pl-10 pr-4 bg-white py-3 border rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all duration-200 ${errors.email ? "border-red-500 bg-red-50" : "border-gray-300"
                }`}
            />
          </div>
          {errors.email && (
        ...
        <div className="relative mb-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              autoComplete="off"
              className={`w-full bg-white pl-10 pr-12 py-3 border rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all duration-200 ${errors.password ? "border-red-500 bg-red-50" : "border-gray-300"
                }`}
            />
            <button
              type="button"
...
const RegisterComponent = ({
  ...
  const [skipReferral, setSkipReferral] = useState(true);
  ...
  const [OTPresent, { isLoading: isOTPresentLoading, error: OTPresentError }] =
    useOTPresentMutation();
  const { data: referralData, isLoading: isReferralLoading, isError: isReferralError } =
    useReferralInfoQuery(
      referralUsername,  // ✅ Just the string
      { skip: skipReferral }
    );
...
useEffect(() => {
  ...
  if (referralCode) {
    ...
    // ✅ Trigger API call if valid format
    const isValidFormat = /^(?=.*[A-Z])(?=.*\d)[A-Z0-9]{13}$/.test(referralCode);
    if (isValidFormat) {
      setReferralUsername(referralCode);
      setSkipReferral(false); // ✅ Trigger API
    } else {
      // Invalid format - show error
      setErrors((prev) => ({
        ...prev,
        referralId: "Invalid Referral ID format",
      }));
    }
  }
}, [location.search]);
  useEffect(() => {
    if (!referralData && !isReferralError) return;

    if (isReferralError) {
      setErrors((prev) => ({
        ...prev,
        referralId: "Invalid referral ID. User not found.",
      }));
      return;
    }
    ...
    if (referralData?.success) {
      ...
      setNotification({
        type: "success",
        message: `Referral applied from: ${referralData?.data?.name || "User"}`,
      });
    }
  }, [referralData, isReferralError]);
  ...
  const getErrorMessage = (error) => {
    if (error) {
      if (error.data && error.data.message) {
        ...
        if (error.status === 400)
          return `Bad Request: ${error.data?.message || "Please check your input."
            }`;
        if (error.status === 401)
          return `Unauthorized: ${error.data?.message || "Invalid credentials."
            }`;
        if (error.status === 409)
          return `Conflict: ${error.data?.message || "User already exists or other conflict."
            }`;
        if (error.status >= 500)
          return `Server Error: ${error.data?.message || "Please try again later."
            }`;
  ...
  const handleInputChange = (e) => {
    ...
    // }
    if (name === "referralId") {
      const isValidFormat = /^(?=.*[A-Z])(?=.*\d)[A-Z0-9]{13}$/.test(value);
      if (isValidFormat) {
        setReferralUsername(value);
        setSkipReferral(false); // ✅ Trigger API call
      } else {
        setSkipReferral(true);  // ❌ Stop API call
        setReferralUsername("");
      }
    }
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      setNotification({ type: "success", message: "OTP sent to your Registered Mobile No!" });
    } catch (err) {
  ...
  return (
    <div
      className={`w-full max-w-md mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-500 transform ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
        }`}
    >
      {notification && (
        <Notification
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1">
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-10 bg-white pr-3 py-2.5 text-sm border rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all duration-200 ${errors.name && touched.name
        ...
        <div className="space-y-0">
          <div
            className={`flex rounded-lg border transition-all duration-200 ${errors.phone && touched.phone
              ? "border-red-500 bg-red-50"
        ...
        <div className="space-y-0">
          <div className="relative">
            ...
            <input
              ...
              onBlur={handleBlur}
              placeholder="Email"
              className={`w-full pl-10 pr-3 py-2.5  bg-white text-sm border rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all duration-200 ${errors.email && touched.email
                ? "border-red-500 bg-red-50"
                : "border-gray-300"
                }`}
            />
        ...
        <div className="space-y-0">
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-10 pr-10 bg-white py-2.5 text-sm border rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all duration-200 ${errors.password && touched.password
                ? "border-red-500 bg-red-50"
                : "border-gray-300"
                }`}
            />
        ...
        <div className="space-y-0">
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-10 pr-10 bg-white py-2.5 text-sm border rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all duration-200 ${errors.confirmPassword && touched.confirmPassword
                ? "border-red-500 bg-red-50"
                : "border-gray-300"
                }`}
            />
        ...
        <div className="space-y-0">
          <div className="relative">
            ...
            <input
              ...
              readOnly={referralApplied}
              placeholder="Referral ID (Optional)"
              className={`w-full pl-10 pr-10 py-2.5 text-sm bg-white border rounded-lg focus:ring-2 
        focus:ring-teal-500 focus:border-teal-500 outline-none transition-all duration-200 ${errors.referralId && touched.referralId
                  ? "border-red-500 bg-red-50"
                  : referralData?.success && formData.referralId
            ...
            {isReferralLoading && (
              <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                <svg className="animate-spin h-4 w-4 text-teal-500"
            ...
            {/* ✅ Success Check */}
            {referralData?.success && formData.referralId && !isReferralLoading && (
              <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
          ...
          {/* ✅ Referral Owner Name */}
          {referralData?.success && !isReferralLoading && (
            <p className="text-green-600 text-xs pl-1">
              Referred by: <strong>{referralData?.data?.name}</strong>
            </p>
          )}
        </div>
        {/* OTP Field */}
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                maxLength="6"
                className={`w-full bg-white pl-10 pr-3 py-2.5 text-sm border rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all duration-200 ${errors.otp && touched.otp
                  ? "border-red-500 bg-red-50"
                  : "border-gray-300"
                  }`}
              />
  ...
  return (
    <div className="min-h-screen w-full overflow-hidden bg-gray-50">
      ...
      <div className="hidden lg:flex w-full h-screen relative">
        ...
        <div
          className={`absolute inset-y-0 w-1/2 bg-gradient-to-br from-[#085358] via-teal-600 to-green-900 transform transition-all duration-1000 ease-out ${isLogin ? "left-0 translate-x-0" : "left-1/2 translate-x-0"
            }`}
        >
          {/* Navigation Tabs */}
          ...
          <div className="relative text-center text-white px-8 flex flex-col items-center justify-center h-full overflow-hidden">
            ...
            <div className="relative mt-6">
              <h2
                className={`text-4xl font-bold mb-4 transition-all duration-700 transform bg-gradient-to-r from-white via-teal-100 to-white bg-clip-text text-transparent ${isTransitioning
              ...
              <p
                className={`text-teal-50 text-lg leading-relaxed transition-all duration-700 transform ${isTransitioning
                  ? "opacity-0 translate-y-4"
                  : "opacity-100 translate-y-0"
                  }`}
              >
      ...
      <div className="lg:hidden w-full min-h-screen flex flex-col">
        ...
        <div className="bg-gradient-to-br from-[#085358] via-teal-600 to-green-900 relative overflow-hidden">
          ...
          <div className="relative z-10 py-8 text-center text-white">
            ...
            <div className="px-4">
              <h2
                className={`text-2xl font-bold mb-2 transition-all duration-700 transform bg-gradient-to-r from-white via-teal-100 to-white bg-clip-text text-transparent ${isTransitioning
                  ? "opacity-0 translate-y-4"
                  : "opacity-100 translate-y-0"
                  }`}
              >
              ...
              <p
                className={`text-teal-50 text-sm leading-relaxed transition-all duration-700 transform ${isTransitioning
                  ? "opacity-0 translate-y-4"
                  : "opacity-100 translate-y-0"
                  }`}
              >
