...
const Profile = () => {
  ...
  // API form data for password change
  ...
  const GetOtp = async () => {
    if (!formData.currentPassword.trim()) {
      setPasswordErrors({ currentPassword: 'Current password is required to get OTP.' });
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const hasChanged =
      formData.name !== userData?.data?.name ||
      formData.address !== userData?.data?.address ||
      formData.city !== userData?.data?.city ||
      formData.state !== userData?.data?.state ||
      formData.country !== userData?.data?.country ||
    ...
    try {
      ...
      apiFormData.append("_id", userData?.data?._id);
      ...
      if (formData.profilePicture instanceof File) {
        apiFormData.append("profile", formData.profilePicture);
      }

      const res = await update(apiFormData);
              ...
              {['name', 'email', 'mobile', 'address', 'city', 'state', 'country'].map((field) => (
                <div key={field}>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    placeholder={`Enter your ${field}`}
                    disabled={field === 'email' || field === 'mobile'}
                    className={`w-full px-3 py-2 rounded border bg-white/10 text-white placeholder-white/60 focus:outline-none focus:ring-2
                      ${errors[field] ? 'border-red-500 ring-red-500' : 'border-white/20 ring-[#4ecdc4]'}
                      ${(field === 'email' || field === 'mobile') ? 'opacity-60 cursor-not-allowed' : ''}`}
                  />
                  {errors[field] && (
                    <p className="mt-1 text-xs text-red-400">{errors[field]}</p>
                  )}
