...
// ─── Modal ────────────────────────────────────────────────────────────────────
export default function SellToCompanyModal({ isOpen, onClose, netTokens = 0, onSellSuccess }) {
  ...
  const [previewSell, { isLoading: previewing }] = usePreviewCompanySellMutation();
  const [executeSell, { isLoading: selling }] = useExecuteCompanySellMutation();
  ...
  const handlePreview = async () => {
    ...
    try {
      const res = await previewSell(amount).unwrap();
  ...
  const handleSell = async () => {
    setErrorMsg("");
    try {
      const res = await executeSell(Number(tokensToSell)).unwrap();
